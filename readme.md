...
|`MDNS_ADVERTISE`|`0`, `1`|`1`|Advertise this device's API on the local network over mDNS as an `_http._tcp` service, so it can be discovered by other devices/Bonjour browsers. <br/> `0` = off, `1` = on|
...
|`MDNS_DISCOVER_TYPE`|`string`|`http`|The DNS-SD service type(s) to browse for, without the leading underscore or `._tcp`. Accepts a comma-separated list to browse several at once, e.g. `http,https`. Use `home-assistant` to discover a Home Assistant server (advertised as `_home-assistant._tcp`, port 8123). The displayed URL scheme is chosen from the matched service's type (`https` for `_https._tcp`, or any service on port 443)|
|`MDNS_DISCOVER_PATH`|`string`|N\A|Path appended to the discovered `host:port` (services only advertise a host and port, never a path), e.g. `/lovelace-tvboard/tvboard` to open a specific Home Assistant dashboard|
...
#### **GET** /mdns
Returns a JSON object describing the mDNS service this device advertises (if `MDNS_ADVERTISE=1`) and the `_http._tcp` services currently visible on the LAN. Example:
```json
{
  "advertising": { "name": "my-kiosk", "type": "_http._tcp", "port": 5011 },
  "discovered": [
    { "name": "grafana", "host": "grafana.local", "port": 3000, "addresses": ["192.168.1.20"], "url": "http://192.168.1.20:3000" }
  ]
}
```
